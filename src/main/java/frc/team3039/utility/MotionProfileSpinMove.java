...
public class MotionProfileSpinMove {
    ...
    public MotionProfileSpinMove(double initialLinearVelocity, double startAngle, double endAngle,
            double maxTurnVelocity, double itp, double t1, double t2) {
    ...
    public MotionProfileTurnPoint getNextPoint(MotionProfileTurnPoint point) {
        ...
        if (point != null) {
            double linearVelocity = initialLinearVelocity
                    * Math.cos(Math.toRadians(point.position * spinAngleScaleFactor));
            double angularVelocity = Constants.kDriveWheelTrackWidthInches * Constants.kTrackScrubFactor
                    * Math.toRadians(point.velocity);
    ...
    public static void main(String[] args) {
        ...
        try {
            out = new PrintWriter("C:/Users/Joshl/turn180.csv");
            long startTime = System.nanoTime();

            MotionProfileSpinMove mp = new MotionProfileSpinMove(-48, 0, -90, 240, 10, 200, 100);
            ...
            while (mp.getNextPoint(point) != null) {
                out.println(point.time + ", " + point.position + ", " + point.velocity + ", "
                        + inchesPerSecondToTicksPer100ms(point.leftVelocity) + ", "
                        + inchesPerSecondToTicksPer100ms(point.rightVelocity));
            }
